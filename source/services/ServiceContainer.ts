...
import {FeatureLayer} from "@evergis/sgis/layers/FeatureLayer";
import {ajaxp, parseJSON} from "../utils";

let serviceTypeRegistry = [];

export class ServiceContainer extends EventHandler {
    _error: any;
    ...
    _loadServiceInfo() {
        ...
        return this._connector.initializationPromise.then(() => { return ajaxp({url}); })
            .then(([response]) => {
